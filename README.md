...
- **Node**: ESP32-S3 SuperMini (or ESP32-S3 DevKitC-1)
...
- **Tested 4 MB Binary**: `ruview/firmware/esp32-csi-node/release_bins/esp32-csi-node-4mb.bin`

### Flashing 4 MB ESP32-S3:
```bash
...
### Provisioning NVS (Wi-Fi & Target IP):
```bash
...
```bash
python backend/scripts/sim_esp32_stream.py --target-ip 127.0.0.1 --target-port 5005 --node-id 1
```
